...
public class BicycleLinkSpeedCalculatorPrecomp implements BicycleLinkSpeedCalculator {
    private static final Logger log = LogManager.getLogger(BicycleLinkSpeedCalculatorPrecomp.class );
    @Inject private BicycleConfigGroup bicycleConfigGroup;
    @Inject private QSimConfigGroup qSimConfigGroup;
    @Inject private Config config;
    @Inject private BicycleLinkSpeedCalculatorPrecomp() {
    }

    /**
     * for unit testing
     */
    BicycleLinkSpeedCalculatorPrecomp( Config config ) {
    ...
    /** Return product of infrastructure * surface * gradient (uncapped). */
    public double getMultiplier(Link link) {
        double infrastructureFactor = this.computeInfrastructureFactor(link);
        double surfaceFactor = this.computeSurfaceFactor(link);
        double gradientFactor = this.computeGradientFactor(link);
        return infrastructureFactor * surfaceFactor * gradientFactor;
    }

    /** expose the default max (used when vehicle == null) */
    public double getDefaultMaxBicycleSpeedForRouting() {
        return this.bicycleConfigGroup.getMaxBicycleSpeedForRouting();
    }
